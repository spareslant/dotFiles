...
function set_base16_tmux_theme() {
  local theme="$1"
  if egrep 'source.+conf$' ${DOT_FILES_LOC}/tmux/tmux.conf > /dev/null
  then
    perl -wpl -s -i -e 's!(^source\s+.+/)(.+?)(\.conf)$!$1$theme$3!' -- -theme="${theme}" ${DOT_FILES_LOC}/tmux/tmux.conf
  else
    echo "source ${DOT_FILES_LOC}/downloaded/extraShellUtilities/base16-tmux/colors/${theme}.conf" >> ${DOT_FILES_LOC}/tmux/tmux.conf
  fi
}
